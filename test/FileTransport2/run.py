...
class PySysTest(ApamaBaseTest):
	def execute(self):
		...
		for f in ['read.txt','read2.txt', 'config.yaml']:
		...
		self.waitForSignal('correlator.log', expr="Hello from transport2", condition=">=10")
		self.waitForSignal('write2.txt', expr="Hello from correlator", condition=">=10")
	...
	def validate(self):
		...
		self.assertOrderedGrep('correlator.log', exprList=[f'Hello from transport2 {i}'  for i in range(0, 10)])
		self.assertOrderedGrep('write2.txt', exprList=[f'Hello from correlator {i}'  for i in range(0, 10)])
